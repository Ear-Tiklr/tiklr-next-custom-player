...
export const useUserStore = create<UserStoreState>()((set) => ({
  ...
  setFavorite: async (music: Music) => {
    const res = await PutUserFavorite(
  ...
  addPlayList: async (payload: PlayList): Promise<StoreResponse<PlayList>> => {
    const { status, error, data } = await PutUserPlayList(
      useUserStore.getState().userInfo.id,
      payload
    );
  ...
  deletePlayList: async (id: number): Promise<StoreResponse<string>> => {
    ...
    if (status === 201) {
      ...
      useAppStore
        .getState()
        .setPlaylist("recent-musics-playlist", useUserStore.getState().recent);
    }
  ...
  setPlayListMusic: async (
    ...
    music: Music,
    playlistId: number
  ): Promise<StoreResponse<string>> => {
    const res = await PutUserPlayListMusic(
      useUserStore.getState().userInfo.id,
      playlistId,
      music,
      type
  ...
  changePlayListMusicOrder: async (playlistId: number, newOrder: Music[]) => {
    const res = await PutPlayListMusicOrder(
      useUserStore.getState().userInfo.id,
      playlistId,
      newOrder
    );
